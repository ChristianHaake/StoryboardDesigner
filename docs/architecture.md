...
| PDF-Download lazy (jspdf + html-to-image) plus separater Druck | Echte `.pdf`-Datei für Download/Weitergabe; Canvas wird seitenweise als JPEG eingebettet; `window.print()` bleibt als Fallback vertraut | DOM→Bild→PDF ist Raster, kein selektierbarer Text; iPad/MDM muss separat getestet werden        |
