  ...
  return (
    <div className="mx-auto max-w-5xl py-8 px-4 sm:px-6 lg:px-8 fade-in">
      <div className="mb-14 mt-4 text-center">
        ...
        <h1 className="mb-6 text-balance text-4xl font-extrabold tracking-tight text-slate-900 sm:text-6xl">
          {t('wizard.startTitlePrefix')}{' '}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">
            {t('wizard.startTitleEmphasis')}
          </span>
        </h1>
        <p className="mx-auto max-w-2xl text-pretty text-lg leading-relaxed text-slate-500 sm:text-xl">
      ...
      <div
        ...
        className="mb-6 grid grid-cols-2 gap-2 rounded-[20px] border border-slate-200 bg-white p-2 sm:grid-cols-4"
      >
      ...
      >
        {visibleTiles.map((tile) => {
          ...
          return (
            <article
              key={tile.type}
              className="group relative flex flex-col items-start rounded-2xl border border-slate-200 bg-white p-6 text-left shadow-sm transition-[border-color,box-shadow] duration-200 hover:border-blue-500 hover:shadow-md hover:ring-1 hover:ring-blue-500"
            >
              <button
                type="button"
              ...
              >
                <div className="flex w-full items-center justify-between">
                  ...
                  <ArrowRight className="h-5 w-5 text-slate-300 opacity-0 transition-[color,opacity,translate] duration-200 group-hover:translate-x-1 group-hover:text-blue-500 group-hover:opacity-100" />
                </div>
                <div className="mt-5">
                  <h3 className="text-balance text-lg font-semibold text-slate-900 transition-colors duration-200 group-hover:text-blue-600">
                    {tile.title}
                  </h3>
              ...
              {hasStarter && (
                <button
                  ...
                  className="mt-4 inline-flex min-h-11 items-center rounded-lg bg-slate-100 px-3 text-sm font-semibold text-slate-700 transition-[color,background-color,transform] motion-safe:active:scale-[0.96] hover:bg-blue-50 hover:text-blue-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
                >
